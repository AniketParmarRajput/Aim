...
export default function OrderDetail() {
  ...
  useEffect(() => {
    if (order?.userId) {
      fetch(
        `${process.env.NEXT_PUBLIC_API_URL}/api/employees/get/${order.userId}`
      )
        .then((r) => r.json())
        .then((res) => {
          if (res.success) {
  ...
  const image = order?.image
    ? (() => {
        const u = Array.isArray(order.image)
          ? order.image[0]
          : order.image;

        return u?.startsWith("http")
          ? u
          : `${process.env.NEXT_PUBLIC_API_URL}/uploads/${u}`;
      })()
    : null;
    ...
    return (
      <div className="min-h-screen bg-brand-cream flex items-center justify-center px-4">
        <div className="text-center animate-fade-in">
          ...
          <button
            onClick={() =>
              router.push("/Common/pages/Management")
            }
            className="mt-6 bg-brand-orange text-white text-[13px] font-semibold px-6 py-2.5 rounded-xl transition-all"
          >
            Back to Admin Panel
          </button>
        </div>
  ...
  const handleDownloadBill = async () => {
    try {
      ...
      const { downloadOrderBill } = await import(
        "@/app/Common/pages/Orders/bill"
      );
    ...
    <div className="min-h-screen bg-brand-cream px-4 py-8">
      <div className="max-w-4xl mx-auto">
        ...
        <div className="flex items-center justify-between mb-6">
          ...
          <button
            onClick={() => router.back()}
            className="inline-flex items-center gap-2 text-[13px] font-medium text-gray-600 bg-brand-light border border-gray-200 px-4 py-2.5 rounded-xl hover:border-brand-orange hover:text-brand-orange transition-all"
          >
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              ...
              className="inline-flex items-center gap-1.5 text-[12px] font-semibold text-green-700 bg-green-50 border border-green-600 px-4 py-2.5 rounded-xl hover:bg-green-600 hover:text-white transition-all"
            >
              🧾 Download Bill
            </button>

            <span
              className={`text-[11px] font-semibold px-3 py-1.5 rounded-full border capitalize ${
        ...
        <div className="bg-brand-light border border-gray-200 rounded-2xl shadow-lg overflow-hidden">
          ...
          <div className="grid grid-cols-1 md:grid-cols-[280px_1fr]">
            ...
            <div className="flex flex-col items-center justify-center gap-4 p-6 border-b md:border-b-0 md:border-r border-gray-200 bg-brand-cream/50">
              ...
              <h3 className="text-center text-[15px] font-bold text-gray-900">
                {order.itemName}
              </h3>
