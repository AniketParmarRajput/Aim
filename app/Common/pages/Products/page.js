...
const BADGE_FILTERS = ["All", "New", "Sale", "Discounted"];
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      <div className="bg-white border-b border-gray-200 px-6 py-4 sticky top-0 z-30 flex items-center justify-between">
        ...
        <button onClick={() => setFilterOpen(true)} className="flex items-center gap-1.5 text-sm font-medium text-gray-600 bg-gray-100 hover:bg-gray-200 px-3 py-2 rounded-lg transition-colors">
          <span>⚙️</span> Filters
        ...
        {filtered.map((item) => (
          ...
          >
            <div className="relative aspect-square bg-gray-50 flex items-center justify-center text-4xl">
              ...
              {item.badge === "new" && <span className="absolute top-2 left-2 bg-green-500 text-white text-[10px] font-bold px-2 py-0.5 rounded-full">New</span>}
              {item.badge === "sale" && <span className="absolute top-2 left-2 bg-red-500 text-white text-[10px] font-bold px-2 py-0.5 rounded-full">Sale</span>}
            ...
            <div className="p-3">
              ...
              <div className="flex items-center gap-2 mt-2">
                <span className="text-lg font-bold text-gray-900">₹{Number(item.amount).toLocaleString("en-IN")}</span>
                {Number(item.discount) > 0 && (
                  <span className="text-xs text-gray-400 line-through">
                    ₹{Math.round(Number(item.amount) / (1 - Number(item.discount) / 100)).toLocaleString("en-IN")}
                  </span>
                )}
              </div>
              <div className="flex gap-2 mt-3">
                <button onClick={(e) => { e.stopPropagation(); addToCart({ id: item.id, itemName: item.itemName, amount: item.amount, image: item.image }); }} className="flex-1 bg-brand-orange hover:bg-brand-orange-hover text-white text-[11px] font-medium py-1.5 rounded-full transition-colors">Add to cart</button>
                <button onClick={(e) => { e.stopPropagation(); router.push(`/Common/pages/Products/${item.id}`); }} className="flex-1 bg-brand-dark hover:bg-[#0a1230] text-white text-[11px] font-medium py-1.5 rounded-full transition-colors">Buy now</button>
              </div>
            </div>
          </div>
        ))}
      ...
      {filterOpen && (
        <>
          ...
          <div className="fixed top-0 right-0 h-full w-72 bg-white shadow-xl z-50 p-5 overflow-y-auto">
